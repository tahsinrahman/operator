...
description: Vault operator Upgrade
menu:
...
# Upgrading Vault operator

If you are upgrading Vault operator to a patch release, please reapply the [installation instructions](/docs/setup/install.md). That will upgrade the operator pod to the new version and fix any RBAC issues.
...
The format for `Restic` object has changed in backward incompatiable manner between 0.5.x and 0.1.0 . The steps involved in upgrading Vault operator to 0.1.0 from prior version involves the following steps:
...
3. Uninstall old `Vault operator` operator.
4. Move repositories to new location if you want to keep your old backups. We have changed repository location in new version of `Vault operator` to remove conflicts between  repositories for different target workloads.
...
5. Install new `Vault operator` operator.
6. Update your backed up `Restic` CRDs in the following ways:
- `restic.spec.useAutoPrefix` is removed in new version of `Vault operator`. If you specify it in your old `Restic` CRD, you should remove it.
...
Say, you are running `Vault operator` operator `0.5.1`.
...
A restic-repository is initialized in `demo` folder of your `stash-qa` bucket and so far 3 backups are created and stored there. Now, the following steps will upgrade version of `Vault operator` without loosing those 3 backups.
...
Now wait for sidecar to be removed by `Vault operator` operator.
...
Uninstall old `Vault operator` operator by following the instructions [here](/docs/setup/uninstall.md).
...
Install new `Vault operator` operator by following the instructions [here](/docs/setup/install.md).
